...
def customer_mart_calculation_table_write(final_customer_data_mart_df):
    ...
    final_customer_data_mart = (
        final_customer_data_mart_df.withColumn("sales_date_month",substring(col("sales_date"),1,7))\
        .withColumn("sales_date_month", concat(col("sales_date_month"), lit("-01")))  
        .withColumn("sales_date_month", to_date(col("sales_date_month"), "yyyy-MM-dd"))  
        ...
        .alias("full_name"),"address","phone_number","sales_date_month",col("total_sales_every_month_by_each_customer").alias("total_sales")).distinct())
